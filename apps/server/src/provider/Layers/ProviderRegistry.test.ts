...
// The Claude PTY driver yields `PtyAdapter` when it materializes. These tests
// keep claudePty disabled, so its probe never spawns and `spawn` is never
// called - a stub that dies if invoked is enough to satisfy the layer graph.
const TestPtyAdapterLive = Layer.succeed(PtyAdapter, {
  spawn: () => Effect.die("PtyAdapter is not available in ProviderRegistry tests"),
});
